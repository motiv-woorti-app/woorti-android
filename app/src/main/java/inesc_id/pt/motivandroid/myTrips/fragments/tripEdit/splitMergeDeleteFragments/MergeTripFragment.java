...
import inesc_id.pt.motivandroid.myTrips.sort.SortFullTripDigestWrappers;
...
import static inesc_id.pt.motivandroid.utils.NumbersUtil.areFullTripDigestsConsecutive;

/**
 * MergeTripFragment
 *
 * Allows the user to choose one or more trips to merge into one trip
 **
 ...
 */
public class MergeTripFragment extends Fragment implements View.OnClickListener {
    ...
    public MergeTripFragment() {
        // Required empty public constructor
    }

    MergeDeleteTripsAdapter adapter;

    ArrayList<FullTripDigestWrapper> toBeMerged;
     ...
     * @return A new instance of fragment MergeTripFragment.
     */
    // TODO: Rename and change types and number of parameters
    public static MergeTripFragment newInstance(String param1, String param2) {
        MergeTripFragment fragment = new MergeTripFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_merge_trip, container, false);

        ((DeleteSplitMergeTripsActivity) getActivity()).setTitleText(getString(R.string.Merge));
        ...
        if(digests.size() != 0){
            ...
            mergeTripPartsButton.setAlpha(0.35f);
            mergeTripPartsButton.setClickable(false);
    ...
    public void checkIfConsecutiveTripsAndGoToConfirmationFragment() {

        toBeMerged = adapter.getToBeMergedOrDeleted();

        SortFullTripDigestWrappers sort = new SortFullTripDigestWrappers();
        Collections.sort(toBeMerged, sort);

        if(toBeMerged.size() < 2){
            Toast.makeText(getContext(), getString(R.string.You_Must_Choose_Two_Trips_To_Merge), Toast.LENGTH_LONG).show();
        }else if(!areFullTripDigestsConsecutive(toBeMerged)){
            Toast.makeText(getContext(), getString(R.string.You_Must_Choose_Consecutive_Trips_To_Merge), Toast.LENGTH_LONG).show();
        }else{
            //if selected trips are valid to be merge, proceed to the confirm operation on map
            //fragment
            FragmentTransaction ft = getFragmentManager().beginTransaction();
            Fragment fragment = MergeTripsConfirmOnMapFragment.newInstance(toBeMerged);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.mergeTripPartButton:
                ...
                checkIfConsecutiveTripsAndGoToConfirmationFragment();
